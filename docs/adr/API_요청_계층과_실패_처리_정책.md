...
HTTP layer는 API별 method, url, request DTO, response DTO, backend app error code, 화면 상태, 사용자 메시지, 재시도 UI를 알지 않는다.
...
- backend error response의 기본 형태를 검증하고 앱 실패 코드로 해석한다.
