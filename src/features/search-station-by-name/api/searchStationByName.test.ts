...
import { createHttpFailure } from "@/shared/api/httpFailure";
import { httpClient } from "@/shared/api/httpClient";
import { searchStationByName, type SearchStationByNameResult } from "./searchStationByName";

describe("searchStationByName", () => {
    afterEach(() => {
        vi.restoreAllMocks();
    });

    it("주유소 이름 검색 결과를 반환한다", async () => {
        const stations = [createSearchStationByNameResult()];
        vi.spyOn(httpClient, "get").mockResolvedValue(createAxiosResponse({ stations }));

        await expect(searchStationByName({ osnm: "보라매", area: "01" })).resolves.toEqual(stations);
    });

    it("응답 body가 기대 형식이 아니면 INVALID_RESPONSE를 던진다", async () => {
        vi.spyOn(httpClient, "get").mockResolvedValue(createAxiosResponse({ items: [] }));

        await expect(searchStationByName({ osnm: "보라매" })).rejects.toMatchObject({
    ...
    it("AbortSignal과 query params를 axios config로 전달한다", async () => {
        ...
        await searchStationByName({ osnm: "보라매", area: "01", signal });
...
function createSearchStationByNameResult(): SearchStationByNameResult {
    return {
        id: "A0001145",
