...
const SEARCH_STATION_NAME_PAGE = "/search-station-name";
const STATION_NAME_API = "**/api/stations/name**";
...
test.describe("주유소명 검색 에러 처리", () => {
    test("재시도 가능한 서버 오류를 안내하고 같은 검색어로 복구한다", async ({ page }) => {
        ...
        await page.route(STATION_NAME_API, async (route) => {
        ...
        await openSearchStationNamePage(page);
        ...
        await expect(page).toHaveURL(new RegExp(`${SEARCH_STATION_NAME_PAGE}$`));
...
async function openSearchStationNamePage(page: Page) {
    await page.goto(SEARCH_STATION_NAME_PAGE);
